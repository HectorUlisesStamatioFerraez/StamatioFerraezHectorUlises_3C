...
const getPokemon = () => {
    $.ajax({
		...
		selectPokemon2.append('<option>Selecciona un Segundo Pokemon...</option>');
        for(let i = 0; i < listPokemon.length; i++){
            selectPokemon.append(`<option value="${ i + 1 }">${ listPokemon[i].name }</option>`);
			selectPokemon2.append(`<option value="${ i + 1 }">${ listPokemon[i].name }</option>`);
...
const create = () => {
	...
	person.favorite2 = selectPokemon2;
...
const getUserByIndex = (index) => {
    ...
    for(let i = 0; i < persons.length; i++){
        if(i === index){
			...
			document.getElementById("#img").value = persons[i].img;
